...
| `STAPEL_WORKSPACES["INVITATION_RESEND_COOLDOWN_SECONDS"]` | tuning | `600` | Seconds before an invited address may be mailed about its invitation again (`POST <ws>/invitations/<id>/resend` → 429). A duration, not a DRF rate, because the window belongs to the recipient's inbox and not to the calling admin; `0`/`None` disables, a malformed value is `stapel_workspaces.E009` at `manage.py check` |
| `STAPEL_WORKSPACES["INVITATION_ROTATE_TOKEN_ON_RESEND"]` | behaviour | `False` | Whether a resend mints a new token and kills the link already in the invitee's mailbox. Off since 0.23 (it was unconditional before): the resend goes to the same address, so rotation narrows nothing and turns the letter the invitee is most likely to click into a 404. Revoke + re-invite is the answer when a token is believed leaked |
...
| `STAPEL_WORKSPACES["AUDIT_SINK"]` | seam (dotted path, `no_env`) | `stapel_workspaces.audit.eventstore_sink` | Where every membership-journal line goes — `callable(stream, payload, *, project, container)`, the SAME contract as `STAPEL_GATEWAY["AUDIT_SINK"]`, so one custom sink (SIEM shipper, syslog) serves both seams. The default appends to `stapel_core.eventstore` and flushes (membership transitions are read back by admin UIs immediately; the per-process write buffer would otherwise hide a line from a sibling worker). A deployment that swaps the sink away from the event store takes over serving history too — `GET <id>/audit` reads the store only |
| `STAPEL_WORKSPACES["AUDIT_STREAM"]` | axis | `"workspace.audit"` | The event-store stream the journal is written to and read from (one stream per journal; the workspace is the `workspace_id` payload filter). Retention/rollups/backends key on this name via `STAPEL_EVENTSTORE` (`RETENTION`, `ROUTES`); `manage.py audit_trail` discovers it by the `*.audit` convention |
