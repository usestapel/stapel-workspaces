...
| Events (`events.py`, `schemas/`) | `EVENT_WORKSPACE_PERSONAL_CREATED = "workspace.personal.created"`, `EVENT_WORKSPACE_MEMBER_REMOVED` / `EVENT_WORKSPACE_MEMBER_ROLE_CHANGED` (member lifecycle, spec §A4), `EVENT_WORKSPACE_MEMBER_PROVISIONED` / `EVENT_WORKSPACE_MEMBER_SUSPENDED` / `EVENT_WORKSPACE_MEMBER_UNSUSPENDED` (security harden, spec §C1/§C3), payload dataclasses, `EVENT_REGISTRY`; JSON Schemas in `schemas/emits/`, `schemas/consumes/`, `schemas/functions/` |
